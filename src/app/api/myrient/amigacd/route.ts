...
export async function GET() {
  try {
    const response = await fetch(
      "https://myrient.erista.me/files/Redump/Commodore%20-%20Amiga%20CD/",
    ...
    for (const row of rows) {
      ...
      if (linkMatch) {
        ...
        else if (fileName.includes("(UK)")) region = "UK";
        else if (fileName.includes("(Poland)")) region = "Poland";
        else if (fileName.includes("(Sweden)")) region = "Sweden";

        const id = fileName.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 32);
        const base = "https://myrient.erista.me/files/Redump/Commodore%20-%20Amiga%20CD/";
    ...
    console.error("Error fetching Myrient Amiga CD ROMs:", error);
