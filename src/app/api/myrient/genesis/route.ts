...
const BASE_URL = "https://myrient.erista.me/files/RetroAchievements/RA%20-%20Sega%20Genesis/";

export async function GET() {
  try {
    const response = await fetch(BASE_URL, { next: { revalidate: 3600 } });
    const html = await response.text();
    const { roms, regions } = parseMyrientRoms(html, BASE_URL, "genesis", "Sega Genesis");
    return NextResponse.json({ success: true, count: roms.length, regions, roms });
  } catch (error) {
    console.error("Error fetching Myrient Genesis ROMs:", error);
