...
export function HeartsGame() {
  ...
  // Confetti when human wins (lowest score at game end)
  useEffect(() => {
    if (gameState.phase !== "gameEnd" || gameState.players.length === 0) return;
    const minScore = Math.min(...gameState.players.map(p => p.score));
    const humanPlayer = gameState.players.find(p => p.type === "human");
    if (humanPlayer && humanPlayer.score === minScore) {
